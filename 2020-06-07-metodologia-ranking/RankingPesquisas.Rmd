...
refletirem as intenções de voto no momento em que foram realizadas, elas pautam
narrativas de candidatos e da mídia sobre a eleição -- narrativas que, por sua
vez, podem ter um impacto forte sobre a opinião pública.
...
Foi para responder essa pergunta que o _Pindograma_ criou o seu Ranking de
Institutos de Pesquisa. Nossa metodologia, inspirada nos critérios usados pelo
site americano [FiveThirtyEight][1], usa vários medidas para avaliar o
...
tarefa que mais tomou esforço do _Pindograma_. Embora [vários][2] [sites][3]
...
Usando dados do TSE, levantamos todos os institutos de pesquisa que registraram
mais de 15 pesquisas entre 2012 e 2018. Em seguida, procuramos os sites de
todas essas empresas, e quando disponíveis, baixamos todos os relatórios de
...
No começo, esperávamos receber mais relatórios entrando em contato com os
...
##### Busca por notícias que contém resultados de pesquisa
...
Alguns meios de comunicação publicam pesquisas sem número; e outros publicam
esse número de forma pouco ortodoxa. Mas quase sempre, pesquisas que foram
...
"preencheu planilhas" durante três meses para consolidar o levantamento mais
completo de pesquisas para o período 2012-2018.
...
TSE. Mas diferença faz sentido, e se deve a alguns fatores:
...
email para pesquisas@pindograma.com.br.

##### Pós-processamento dos dados

Para garantir a qualidade desses dados, o _Pindograma_ fez uma série de
verificações. Corrigimos pesquisas cujo total somava mais de 100%; sondagens que
estavam associadas às cidades erradas; e outras coisas do gênero. Em seguida,
cruzamos todas as pesquisas que encontramos com a relação de candidatos
divulgada pelo TSE.
...
dados e a remoção de sondagens que não tinham um número do TSE. E destas, `r
pn(number_of_unique_p3_polls)` não foram localizadas no levantamento do
_Pindograma_.
...
ter caído, por puro azar, fora do intervalo de confiança.
...
pesquisas disponíveis na base. Ao invés disso, selecionamos as sondagens que
...
* **Âmbito da pesquisa**. O _Pindograma_ incluiu na análise as nacionais para
  presidente; estaduais para presidente; estaduais para governador; e
  municipais para prefeito. Não incluímos pesquisas para presidente ou
  governador no âmbito municipal. Também excluímos pesquisas cujo âmbito é uma
  região específica, como pesquisas para governador em cidades do litoral de
  São Paulo. (O âmbito de cada pesquisa é manualmente determinado pelo
  _Pindograma_ através de dados do TSE).
...
Esse exemplo foi escolhido de propósito, porque revela as limitações desse
...
* Quantos dias faltam para a eleição;
...
se ela tivesse um erro médio ajustado de 3 pontos, o índice seria de **`r pn(3 -
predicted)`**. Note que **quanto menor esse índice, melhor é o desempenho da
pesquisa**.
...
Mais uma vez, se trata de um exemplo escolhido para demonstrar um problema. O
desempenho desses quatro institutos é parecida pela média do Índice de
...
B, esse problema não existe. Com 848 pesquisas analisadas, é bem mais tranquilo
supor que o índice de -0,578 _realmente_ represente o desempenho do instituto
como um todo.
...
Ranking de Institutos. Mas não é só. Com o Ranking, é possível demonstrar a
desigualdade no acesso a pesquisas eleitorais de qualidade pelo Brasil; elencar
institutos que poderiam ser investigados por publicarem pesquisas de má
qualidade; exigir uma prestação de contas das empresas de pesquisa; e sobretudo,
qualificar o debate sobre pesquisas eleitorais no Brasil.
