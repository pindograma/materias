...
"Alô? Com quem eu falo?" Uma versão acústica de "Ninguém Explica Deus" toca ao
...
1.000 pessoas, segundo dados da Prefeitura do Rio, e analisamos os resultados
das últimas eleições presidenciais:

```{r}
ggplot() +
  ggtitle('Margem de Vitória de Jair Bolsonaro nas Favelas do Rio') +
  labs(subtitle = '(em pontos percentuais)', caption = 'Fonte: TSE') +
...
Bolsonaro obteve maioria em quase todas as favelas do Rio -- o atual presidente
foi vitorioso em `r pn(favelas_bolsonaro_win)` comunidades somando
`r pn(favelas_bolsonaro_moradores)` moradores; enquanto Haddad venceu em somente
`r pn(favelas_haddad_win)` favelas, somando `r pn(favelas_haddad_moradores)`
moradores.
...
plot14 = ggplot(favelas_grouped_ev, aes(x = evangelicos, y = pt_2014)) +
  ...
  xlab('População Evangélica') + ylab('Votos no candidato do PT')
...
inédito. Em 2014, áreas do Rio com maior presença de evangélicos já votavam mais
em candidatos conservadores para o Legislativo; e também já davam mais votos que
os católicos a Marcelo Crivella, um pastor licenciado. Mas foi só em 2018 que o
voto evangélico pesou em uma eleição presidencial.
...
do PT nas favelas. Para o pesquisador Ignacio Cano, "o PT ficou órfão de quadros
locais" nos últimos anos, ele explica que com a economia indo bem, era fácil
convencer líderes locais a fazerem campanha pelo partido; mas que essa base foi
perdida com a crise econômica. Segundo Cano, a coligação de partidos e
...
restringir a atuação de ONGs. A milícia disputa o controle da miséria".

##### III) Milícias

Então a como as milícias afetam o voto do eleitor carioca? Para o pesquisador
...
cabos eleitorais. Da mesma forma, o fato da milícia manter relações duradouras
com quem ela interage tende a facilitar um bom relacionamento com certos
políticos -- coisa que o _modus operandi_ furtivo do tráfico dificulta.
...
Mas apesar de exercerem um controle perigoso sobre os pleitos minoritários nas
favelas onde têm presença, as milícias não aparentam ter produzido vantagem para
nenhum candidato a presidente em 2018. Usando dados do Disque Denúncia do Rio,
classificamos as favelas como áreas de tráfico ou áreas de milícia, baseado em
quantas ligações da região denunciavam cada grupo em 2016 e 2017. E houve pouca
